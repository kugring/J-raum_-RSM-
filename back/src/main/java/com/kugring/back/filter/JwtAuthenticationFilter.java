...
public class JwtAuthenticationFilter extends OncePerRequestFilter { // 해당 클래스를 필터로 사용하기 위해서는 추상메서드를 확장해준다?
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
            throws ServletException, IOException {
                ...
                try {
                    ...
                    String token = parseBearerToken(request); // 아래쪽에 parseBearerToken를 통해서 헤더의 authorization값의 토큰을 가져온다.
                    ...
                    String role = userEntity.getRole(); // role: ROLE_USER, ROLE_ADMIN  스프링 시큐리에 접근하는 권한에 관련된 변수는 접두사인 "ROLE_"를 붙이게 된다.
                    ...
                    AbstractAuthenticationToken authenticationToken =  // 접근 주체에 대한 토큰 정보
                        new UsernamePasswordAuthenticationToken(userId, null, authorities); // 담겨지는 파라마터는 (해당 정보, 비밀번호, 시큐리에 접근하는 권한자의 리스트)
                ...
                filterChain.doFilter(request, response); // 다음 필터로 넘어갈 수 있도록 한다?? 라는게 무슨 말이지...? 이번 필터가 끝나고 이어서 다음 필터로 넘어간다는 얘기인가?
    }
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        boolean hasAuthorization = StringUtils.hasText(authorization); // hasText는 해당 String이 null 아닌지 length가 0은 아닌지 // whiteSpace로 공백은 아닌지 확인해줌
