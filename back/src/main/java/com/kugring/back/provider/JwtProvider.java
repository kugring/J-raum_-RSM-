...
public class JwtProvider {
  ...
  public String create(String userId) {
    Date expiredDate = Date.from(Instant.now().plus(1, ChronoUnit.HOURS)); // 이코드가 해당 토큰을 1시간으로 지정하는
                                                                           // 듯하다!
    Key key = Keys.hmacShaKeyFor(secretKey.getBytes(StandardCharsets.UTF_8)); // 이 코드는
  ...
  public String validate(String jwt) {
    ...
    try {
      subject = Jwts.parserBuilder() // json토큰웹에 대한것 같다?
