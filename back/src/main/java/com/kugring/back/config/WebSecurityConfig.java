...
public class WebSecurityConfig {
    ...
    protected SecurityFilterChain configure(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity
                ...
                .httpBasic(HttpBasicConfigurer::disable) // 여기 프로젝트에서는 Bearer토큰 방식을 사용하기 때문에 기본으로 스프링 시큐리티가 설정해주는
                                                         // httpBasic을 사용하지 않는다.
                .sessionManagement(sessionManagement -> sessionManagement
                        .sessionCreationPolicy(SessionCreationPolicy.STATELESS) // stattless라고 지정하면 세션을 유지하지 않겠다라는 의미이다.
                                                                                // (세션을 이번 프로젝트에서 사용하지 않는다.)
                )
                .authorizeHttpRequests(request -> request
                        .requestMatchers("/", "/api/v1/auth/**", "/oauth2/**", "/api/v1/board/**").permitAll()
                        .requestMatchers("/api/v1/user/**").hasRole("USER")
                        .requestMatchers("/api/v1/admin/**").hasRole("ADMIN")
                        .anyRequest().authenticated() // 위에 경로를 제외한 모든것에서는 권한을 확인하겠다!라는 의미.
                ...
                .oauth2Login(oauth2 -> oauth2
                        ...
                        .successHandler(oAuth2SuccessHandler) // successHandler의 파라미터로 우리가 커스텀한 auth2SuccessHandler이라는 데이터를 전해준다.
                )
    ...
    protected CorsConfigurationSource corsConfigurationSource() {
        ...
        corsConfiguration.addAllowedOrigin("*"); // 모든 출처에 대해서 허용하겠다.
        corsConfiguration.addAllowedMethod("*"); // 모든 메소드에 대해서 허용하겠다.
        corsConfiguration.addAllowedHeader("*"); // 모든 헤더에 대해서 허용하겠다.

        // 반환할 configurationSource이다. UrlBased가 뭘까? 인스턴트 생성이라고 하네?
        UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();

        // 모든경로("/**")에 corsConfiguration에 대한 설정을 적용해준다.
